...
def CogUpdate(cogs, twogs, prots):
    ...
    cog_list = list(set([y.split()[0] for y in cogs]))
    for x in cog_list:
        #percentage begin
        if (cog_list.index(x) + 1) % round(len(cog_list)/100) == 0:
            os.system("clear")
            org = prots[0].split("_")[0]
            found = False
            for y in VOORTGANG:
                if y != CURRENT_ORG:
                    if found == False:
                        print(y + "\t" + "COG_UPDATE\t100%")
                        print("\t" + "NEW_COG_FIND\t100%")
                    else:
                        print(y + "\t" + "COG_UPDATE\t0%")
                        print("\t" + "NEW_COG_FIND\t0%")
                else:
                    print(y + "\t" + "COG_UPDATE\t" + str(round((cog_list.index(x) + 1)/(len(cog_list)) * 100)) + "%")
                    print("\t" + "NEW_COG_FIND\t0%")
                    found = True
        #percentage eind
...
def NewCogFind(ncf_cogs, ncf_twogs, ncf_prots):
    ...
    for prot in ncf_prots:
        #percentage begin
        if (ncf_prots.index(prot) + 1) % round(len(ncf_prots)/100) == 0:
            os.system("clear")
            org = ncf_prots[0].split("_")[0]
            found = False
            for x in VOORTGANG:
                if x != CURRENT_ORG:
                    if found == False:
                        print(x + "\t" + "COG_UPDATE\t100%")
                        print("\t" + "NEW_COG_FIND\t100%")
                    else:
                        print(x + "\t" + "COG_UPDATE\t0%")
                        print("\t" + "NEW_COG_FIND\t0%")
                else:
                    print(x + "\t" + "COG_UPDATE\t100%")
                    print("\t" + "NEW_COG_FIND\t" + str(round((ncf_prots.index(prot) + 1)/(len(ncf_prots)) * 100)) + "%")
                    found = True
        #percentage eind
...
def OpenNeededFiles(onf_org):
    protlijst = [y[1:-1] for y in OpenFile("prots/%s.fa"%(onf_org)) if ">" in y]
